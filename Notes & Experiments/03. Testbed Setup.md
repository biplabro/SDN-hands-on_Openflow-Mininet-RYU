...
It is **NOT ADVISED to change** the default python2.7 installation that comes built-in with Ubuntu, changing the python setting **might break** the overall integrity of the system. We can install both the python2 & python3 & as per our requirements we can always call the mininet scripts using `python mininet_script.py` or `python3 mininet_script.py`. This is **TESTED** & should work perfectly.
...
`python mininet_script.py` will actually call `python3 mininet_script.py` & the latest & greatest version of mininet(2.3.0d6) (which is **Installed from GitHub source only**, not from the apt package manager) will be execyted on top of python3 environment.
...
`async` is a reserved keyword in python3.7 and an old version of pexpect is using _async as variable_. The solution is to upgrade pexpect.
